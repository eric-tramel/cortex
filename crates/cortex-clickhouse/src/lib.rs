...
pub fn bundled_migrations() -> Vec<Migration> {
    vec![
        ...
        Migration {
            version: "007",
            name: "007_categorical_domain_contracts.sql",
            sql: include_str!("../../../sql/007_categorical_domain_contracts.sql"),
        },
    ]
}
...
fn materialize_migration_sql(sql: &str, database: &str) -> Result<String> {
    ...
    text = text.replace("cortex.", &format!("{database}."));
    Ok(text)
}
...
mod tests {
    ...
    fn sql_materialization_rewrites_database() {
        let sql = "CREATE DATABASE IF NOT EXISTS cortex;\nCREATE TABLE cortex.events (x UInt8);";
    ...
    fn identifier_validation_rejects_invalid() {
        assert!(validate_identifier("cortex_01").is_ok());
        assert!(validate_identifier("cortex-db").is_err());
    }
