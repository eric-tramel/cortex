...
impl Service {
    ...
    fn launchd_label(self) -> &'static str {
        match self {
            Self::ClickHouse => "com.eric.moraine.clickhouse",
            Self::Ingest => "com.eric.moraine.ingest",
            Self::Monitor => "com.eric.moraine.monitor",
            Self::Mcp => "com.eric.moraine.mcp",
        }
    }

    fn systemd_unit(self) -> &'static str {
        match self {
            Self::ClickHouse => "moraine-clickhouse.service",
            Self::Ingest => "moraine-ingest.service",
            Self::Monitor => "moraine-monitor.service",
            Self::Mcp => "moraine-mcp.service",
        }
    }
}
...
fn launchd_plist(
    ...
    let mut args = vec![
        moraine_exe.to_string_lossy().to_string(),
        "run".to_string(),
...
fn install_launchd_services(
    ...
    let moraine_exe = current_exe_path()?;

    for service in &services {
        let service = *service;
        let plist_path = agents_dir.join(format!("{}.plist", service.launchd_label()));
        let plist = launchd_plist(service, cfg, paths, cfg_path, &moraine_exe);
...
fn systemd_unit_content(service: Service, cfg_path: &Path, moraine_exe: &Path) -> String {
    let exec_args = vec![
        systemd_escape(&moraine_exe.to_string_lossy()),
        "run".to_string(),
        service.name().to_string(),
        "--config".to_string(),
    ...
    format!(
        "[Unit]
Description=Moraine {name} service
{deps}
[Service]
Type=simple
ExecStart={exec}
...
fn install_systemd_services(
    ...
    for service in &services {
        let unit = systemd_unit_content(*service, cfg_path, &moraine_exe);
...
mod tests {
    ...
    fn systemd_unit_content_has_expected_dependencies() {
        let cfg_path = PathBuf::from("/tmp/moraine.toml");
        let moraine = PathBuf::from("/tmp/moraine");

        let clickhouse = systemd_unit_content(Service::ClickHouse, &cfg_path, &moraine);
        assert!(
            clickhouse.contains("ExecStart=/tmp/moraine run clickhouse --config /tmp/moraine.toml")
        );
        assert!(!clickhouse.contains("After=moraine-clickhouse.service"));

        let ingest = systemd_unit_content(Service::Ingest, &cfg_path, &moraine);
        assert!(ingest.contains("After=moraine-clickhouse.service"));
        assert!(ingest.contains("Requires=moraine-clickhouse.service"));
    }

    #[test]
    fn launchd_plist_contains_label_and_program_args() {
        let mut cfg = AppConfig::default();
        cfg.runtime.root_dir = "/tmp/moraine".to_string();
        cfg.runtime.logs_dir = "/tmp/moraine/logs".to_string();
        let paths = runtime_paths(&cfg);
        let plist = launchd_plist(
            Service::Monitor,
            &cfg,
            &paths,
            Path::new("/tmp/moraine.toml"),
            Path::new("/tmp/moraine"),
        );

        assert!(plist.contains("<string>com.eric.moraine.monitor</string>"));
        assert!(plist.contains("<string>/tmp/moraine</string>"));
        assert!(plist.contains("<string>run</string>"));
        assert!(plist.contains("<string>monitor</string>"));
        assert!(plist.contains("<string>--config</string>"));
        assert!(plist.contains("<string>/tmp/moraine.toml</string>"));
    }
