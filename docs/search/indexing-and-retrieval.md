...
Two stats surfaces are exposed as views. `search_term_stats` computes per-term document counts from `search_postings FINAL`, while `search_corpus_stats` computes corpus-wide document count and summed doc length from `search_documents FINAL`. This keeps stats derivation simple and correctness-first, while MCP retains fallback aggregate queries when needed. [src: sql/004_search_index.sql:L151, sql/004_search_index.sql:L156, sql/004_search_index.sql:L161, rust/codex-mcp/src/main.rs:L572]
