...
Documents are produced by two materialized views, one from `normalized_events` and one from `compacted_expanded_events`. Both filter out rows whose text is only whitespace. This keeps corpus size aligned with retrieval utility and avoids ranking overhead on structural events without textual payloads. [src: sql/004_search_index.sql:L28, sql/004_search_index.sql:L53, sql/004_search_index.sql:L55]
...
Two stats tables are incrementally maintained. `search_term_stats` stores approximate per-term document counts, while `search_corpus_stats` stores corpus-wide document count and summed doc length in a single bucket. Both are fed by MVs and use `SummingMergeTree`, trading strict immediacy for efficient additive maintenance. [src: sql/004_search_index.sql:L147, sql/004_search_index.sql:L154, sql/004_search_index.sql:L162, sql/004_search_index.sql:L170]
