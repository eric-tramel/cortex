...
Treat SQL files as contract surfaces, not migration suggestions. The bootstrap path executes all `.sql` files in sorted order, and start scripts call `init-db` automatically. Any breaking schema change should include compatibility reasoning for existing views and scripts, otherwise operational startup can succeed while semantic correctness regresses. [src: bin/init-db:L100, bin/start-clickhouse:L37]
